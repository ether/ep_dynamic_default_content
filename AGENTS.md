# Agent Guide — ep_dynamic_default_content
...
`ep_dynamic_default_content` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_dynamic_default_content`:
...
* Server: `init_ep_dynamic_default_content`, `loadSettings`, `padDefaultContent`, `shutdown`
