# Contributing to ep_dynamic_default_content

Thanks for helping improve `ep_dynamic_default_content` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_dynamic_default_content` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_dynamic_default_content from your local clone
pnpm run plugins i --path ../ep_dynamic_default_content

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_dynamic_default_content/` are picked up after a server restart.
...
* Etherpad version, ep_dynamic_default_content version, browser + OS
* Steps to reproduce
